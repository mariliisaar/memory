...
public class BoardTest {
    ...
    // Set active player
    public void activePlayer(PlayerTest p) {
        this.player = p;
    }
    ...
    private void createCards(int w, int h) { 
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                CardTest card = new CardTest(x, y);
                cards.add(card);
            }
        }
        addSymbols(w * h);
    ...
    public CardTest getCard(int x, int y) {
        CardTest c = cards.get(width * y + x);
        c.turnCardUp();
        return c;
    }
