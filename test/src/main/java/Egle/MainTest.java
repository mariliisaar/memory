...
public class MainTest{
    ...
    public static void main(String[] args){
        ...
        BoardTest gameBoard = new BoardTest(4,4);
        ...
        while(!finishedGame){ //Let's begin the game!
            for(PlayerTest p:players){ //Every player gets their turn!
        ...
        String winner = "";
        int highScore = 0;
        for(PlayerTest p:players){ //Time to see who won!
    ...
    public static void createPlayer(String playerName){ //The method createPlayer creates a player.
        ...
        if(playerCount == 1 &&(playerName==null || playerName.equals(""))){ //Making sure we get at least one player for the game.
            ...
            PlayerTest player = new PlayerTest(playerName,0,0);
            players.add(player);
        }
    }

    public static void playerTurn(String input, PlayerTest player, BoardTest gameBoard){ //This cycles our turns, managing movement, activation and scoring points.
        if (input.equals("")){ //If an empty line is entered, the player moves in their previous or default direction.
            ...
            player.setDirection(DirectionTest.LEFT, gameBoard);
        } else if (input.equals("d")){
            player.setDirection(DirectionTest.RIGHT, gameBoard);
        } else if (input.equals("w")){
            player.setDirection(DirectionTest.UP, gameBoard);
        } else if (input.equals("s")){
            player.setDirection(DirectionTest.DOWN, gameBoard); //This ends the movement section.
